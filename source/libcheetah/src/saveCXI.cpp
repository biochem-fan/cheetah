...
static void write2DToStackCropping(hid_t dataset, uint stackSlice, T *data, int crop_radius, int original_width, int original_height) {
  ...
  //printf("crop_rad = %d, org_height = %d, org_width=%d\n", crop_radius, original_height, original_width);
  ...
  DETECTOR_LOOP{
	...
	createAndWrite2DDatasetCropping("mask_shared", d.self, global->detector[detID].pixelmask_shared, global->detector[detID].cropRadius,
      ...
      */
      ...
      d.photonMap = create2DStack("photon_map", d.self, global->detector[detID].pix_nx, global->detector[detID].pix_ny, H5T_NATIVE_FLOAT);
    }
...
void writeCXI(cEventData *info, cGlobal *global ){
  ...
  if(info->writeFlag){
	...
	write2DToStack(cxi->entry.instrument.detectors[imgID].photonMap, stackSlice, info->detector[detID].photonMap);
    }
